...
void init(){
    ...
    kernel=ch::Worker("sph.cl");
}

cl::Event callKernel(const std::string name){
    return kernel.call(name,listx,listy,listvx,listvy,listfx,listfy,listrho,listp,G_x,G_y,REST_DENS,GAS_CONST,H,HSQ,MASS,VISC,DT,POLY6,SPIKY_GRAD,VISC_LAP,BOUND_SIZE,BOUND_DAMPING,(float)SIM_WIDTH,(float)SIM_HEIGHT,num_particles);
}
